...
class _WaterIntakeListState extends State<WaterIntakeList> {
  ...
  Future<void> _deleteEntry(String docId) async {
    await FirebaseFirestore.instance.collection('water_intake').doc(docId).delete();
  }

  // Update an entry
  Future<void> _updateEntry(String docId, String newLiters) async {
    await FirebaseFirestore.instance.collection('water_intake').doc(docId).update({
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
          ...
          Container(
            decoration: const BoxDecoration(
              image: DecorationImage(
                image: AssetImage('assets/Profile_Screen.png'), // Add your background image here
                fit: BoxFit.cover,
              ),
            ),
          ),
                                ...
                                trailing: Row(
                                  ...
                                  children: [
                                    IconButton(
                                      ...
                                      onPressed: () {
                                        _showUpdateDialog(context, docs[index].id, data['liters'].toString());
                                      },
                                    ),
                                    IconButton(
                                      icon: const Icon(Icons.delete, color: Colors.blue),
                                      onPressed: () => _deleteEntry(docs[index].id),
                                    ),
                                  ],
                                ),
                              ),
      ...
      floatingActionButton: Padding(
        padding: const EdgeInsets.only(bottom: 70), // Adjust position of the plus button
        child: FloatingActionButton(
          ...
          onPressed: () {
            ...
            Navigator.push(
              context,
              MaterialPageRoute(builder: (context) => WaterIntakeForm()),
            );
          },
        ),
      ),
  ...
  void _showUpdateDialog(BuildContext context, String docId, String currentLiters) {
    final TextEditingController _litersController = TextEditingController(text: currentLiters);
      ...
      builder: (context) {
        return AlertDialog(
          ...
          actions: [
            TextButton(
              onPressed: () {
                _updateEntry(docId, _litersController.text);
                Navigator.pop(context);
              },
